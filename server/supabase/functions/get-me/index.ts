...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await drizzleDb.rls((tx) => tx.select().from(users).where(eq(users.id, user.id)));
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/get-me' \
